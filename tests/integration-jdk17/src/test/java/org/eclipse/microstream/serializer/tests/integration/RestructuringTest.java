...
import org.eclipse.microstream.serializer.persistence.binary.types.BinaryLegacyTypeHandler;
...
class RestructuringTest {
    ...
    void loadOldStructure() throws URISyntaxException, IOException {
        URL url = getClass().getClassLoader().getResource("OldAddress.txt");
        Assertions.assertThat(url).as("Cannot find file 'OldAddress.txt' within classpath").isNotNull();

        List<String> lines = Files.readAllLines(Paths.get(url.toURI()));
        byte[] bytes = Base64.getDecoder().decode(lines.get(0));
        ...
        Assertions.assertThat(newAddress.getId()).isEqualTo(123);
        Assertions.assertThat(newAddress.getStreet()).isEqualTo("somewhere"); // should be 'to nowhere'
        Assertions.assertThat(newAddress.getCity()).isEqualTo("666"); // should be 'somewhere'
        Assertions.assertThat(newAddress.getPostalCode()).isEqualTo("to nowhere"); // should be '666'

    }

    @Test
    @Disabled // Binary value created with previous version using a different package name.
    void loadOldStructure_WithTypeDictionary() throws URISyntaxException, IOException {
        URL url = getClass().getClassLoader().getResource("OldAddress.Typed.txt");
        Assertions.assertThat(url).as("Cannot find file 'OldAddress.Typed.txt' within classpath").isNotNull();
        ...
        try (Serializer<byte[]> serializer = TypedSerializer.Bytes(foundation)) {
            // It is able to deserialise the bytes since th TypeDictionary is included and
            // automatic mapping can be performed in this case (small changes)
            newAddress = serializer.deserialize(bytes);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        Assertions.assertThat(newAddress.getId()).isEqualTo(123);
        Assertions.assertThat(newAddress.getStreet()).isEqualTo("to nowhere");
        Assertions.assertThat(newAddress.getCity()).isEqualTo("somewhere");
        Assertions.assertThat(newAddress.getPostalCode()).isEqualTo("666");
    }

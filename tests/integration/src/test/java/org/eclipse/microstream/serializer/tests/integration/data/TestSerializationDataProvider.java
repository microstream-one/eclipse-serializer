package org.eclipse.microstream.serializer.tests.integration.data;
...
import org.eclipse.microstream.serializer.tests.model.*;
...
public class TestSerializationDataProvider implements ArgumentsProvider {
    ...
    public Stream<? extends Arguments> provideArguments(ExtensionContext extensionContext) throws Exception {
        CompareInstances defaultCompare = (i1, i2) -> Assertions.assertThat(i1).isEqualTo(i2);
        CompareInstances toStringCompare = (i1, i2) -> Assertions.assertThat(i1.toString()).isEqualTo(i2.toString());
        CompareInstances arrayDequeCompare = (i1, i2) -> Assertions.assertThat(new ArrayList<>((Collection) i1)).isEqualTo(new ArrayList<>((Collection) i2));
        CompareInstances identityHashMapCompare = (i1, i2) -> Assertions.assertThat(new HashMap<>((Map) i1)).isEqualTo(new HashMap<>((Map) i2));
        return Stream.of(
                ...
                , Arguments.of(new TestSerializationData(BigInteger.valueOf(Integer.MAX_VALUE).add(BigInteger.valueOf(12345)), "BigInteger.test.txt", defaultCompare))
                , Arguments.of(new TestSerializationData(BigDecimal.valueOf(15).pow(24).divide(BigDecimal.valueOf(3)), "BigDecimal.test.txt", defaultCompare))
    ...
    private LocalDateTime defineLocalDateTime() {
        return Instant.ofEpochMilli(MOMENT_IN_TIME).atZone(ZoneId.systemDefault()).toLocalDateTime();
    }

    private ZonedDateTime defineZonedDateTime() {
        return Instant.ofEpochMilli(MOMENT_IN_TIME).atZone(ZoneOffset.of("+01:00").normalized());
    }

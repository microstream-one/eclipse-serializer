package org.eclipse.microstream.serializer.tests.restrictions;
...
public class LambdaRestrictionTest {
    ...
    void failure2() {
        ...
        try (Serializer<byte[]> serializer = Serializer.Bytes(foundation)) {
            Assertions.assertThatThrownBy(
                    ...
                    .hasMessage("Type not persistable: \"class org.eclipse.microstream.serializer.tests.restrictions.LambdaRestrictionTest$1\". Details: Synthetic classes ($1 etc.) are not reliably persistable since a simple reordering of source code elements would change the name identity of a class. For a type system that has to rely upon resolving types by their identifying name, this would silently cause a potentially fatal error. If handling synthetic classes (e.g. anonymous inner classes) is absolutely necessary, a custom org.eclipse.microstream.serializer.persistence.types.PersistenceTypeResolver can be used to remove the exception and assume  complete responsibility for correctly handling synthetic class names.");
    ...
    void success() {
        ...
        try (Serializer<byte[]> serializer = Serializer.Bytes(foundation)) {
            ...
            Assertions.assertThat(newFunc.apply(2)).isEqualTo(4);
